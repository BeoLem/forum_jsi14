...
const jwtToken = config.get('auth.jwt') as string
...
export const GenerateToken = async (
    ...
    const session = await addDoc(
        ...
        new CSession(
            ...
            parseInt(
                type == 'accessToken'
                    ? ms(config.get('auth.accessTokenExpires' as string))
                    : ms(config.get('auth.refreshTokenExpires' as string))
            )
        )
    )
    ...
    return {
        ...
        token: jwt.sign(payload, jwtToken, {
            expiresIn:
                type == 'refreshToken'
                    ? (config.get('auth.refreshTokenExpires') as string)
                    : (config.get('auth.accessTokenExpires') as string),
        }),
        session,
    }
}
